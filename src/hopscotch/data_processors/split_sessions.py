...
from hopscotch.utils.session_detection import build_session_ctes, sql_list, to_list
...
class SplitSessions(DataProcessor):
    ...
    def _validate(self) -> None:
        ...
        if self.timeout is None and boundary_count == 0:
            raise PreprocessingConfigError(
                PROCESSOR_NAME,
    ...
    def apply(self, df: pd.DataFrame, schema: EventstreamSchema) -> Tuple[pd.DataFrame, EventstreamSchema]:
        ...
        session_ctes = build_session_ctes(
            group, path_id_col, event_col, ts_col, subindex_col, separator_starts=True
        )
        ...
        SELECT
            ...
            CASE WHEN w._in_session = 1
                 THEN CONCAT(CAST(w.{path_id_col} AS VARCHAR), '_', CAST(w._session_counter AS VARCHAR))
            END AS {self.session_col}
        FROM with_session_id w
        {where_clause}
        ORDER BY w.{path_id_col}, w.{ts_col}, w.{subindex_col}
        """

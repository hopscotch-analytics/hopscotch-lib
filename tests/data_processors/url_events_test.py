...
from hopscotch.data_processors.url_events import (
    UrlEvents,
    _normalize_path,
...
from hopscotch.exceptions import PreprocessingConfigError, PreprocessingColumnNotFoundError
...
NODES_EXCEPTION_CUT = [
    ...
    {"path": "devices/models", "is_cut": True, "custom_name": "device models","is_deleted": False},
]


def _event(proc: UrlEvents, path: str) -> str | None:
    """Normalize path then return just the event from _transform_path."""
    ep = _parse_effective_path(path, strip_host=False, strip_cgi=False, strip_locale=False)
...
class TestNormalization:
    ...
    @pytest.mark.parametrize("url,kw,expected_path,expected_host,expected_query,expected_locale", [
        ...
        ("https://x.com/en/shop",       {"strip_locale": False},             "en/shop",     "x.com",       "",       "en"),
        # strip_host=False, strip_cgi=True
        ("/shop/cart?foo=bar",          {"strip_host": False},               "shop/cart",   "",            "foo=bar",""),
        # strip_host=False, strip_cgi=False: query stays in path
        ("/shop/cart?foo=bar",          {"strip_host": False, "strip_cgi": False}, "shop/cart?foo=bar", "", "foo=bar",""),
        # locale extraction with strip_locale=False
        ("/fr-ca/products",             {"strip_host": False, "strip_locale": False}, "fr-ca/products", "", "", "fr-ca"),
    ])
    def test_parse_url_parts(self, url, kw, expected_path, expected_host, expected_query, expected_locale):
...
class TestExceptionCut:
    ...
    def test_inner_cut_deep_unknown_child(self):
        assert _event(self.proc, "devices/models/x/y/z") == "devices/models/device models"
...
class TestSlugEnabled:
    ...
    def test_slug_enabled_via_eventstream(self):
        ...
        res = stream.url_events(column="page_url", nodes=NODES_EXCEPTION_CUT, slug_enabled=False)
        assert res.df["event"].tolist() == ["page_view:/devices/abc", "page_view:/devices/models/abc"]
...
class TestExtractionColumns:
    ...
    def test_locale_col_populated(self):
        ...
        res = stream.url_events(column="page_url", nodes=NODES_BASIC, locale_col="locale")
    ...
    def test_locale_col_populated_even_when_strip_locale_false(self):
        ...
        res = stream.url_events(
            column="page_url", nodes=NODES_BASIC,
            strip_locale=False, locale_col="locale",
        )
    ...
    def test_slug_col_populated_for_case_b(self):
        stream = self._stream([
            ...
            ["u1", "/A/D",   "page_view", "2024-01-02"],   # Case B → slug "custom-name"
            ["u1", "/A/D/E", "page_view", "2024-01-03"],   # Case B deep → slug "custom-name"
        ])
    ...
    def test_all_cols_together(self):
        ...
        res = stream.url_events(
            column="page_url", nodes=NODES_BASIC,
            host_col="host", cgi_col="qs",
            locale_col="locale", slug_col="slug",
        )
    ...
    def test_exception_cut_slug_col(self):
        ...
        res = stream.url_events(
            column="page_url", nodes=NODES_EXCEPTION_CUT, slug_col="slug",
        )
...
class TestUrlEventsApply:
    ...
    def test_strip_host_strips_scheme_and_domain(self):
        stream = _make_stream([["u1", "https://example.com/A/B", "page_view", "2024-01-01"]])
        res = stream.url_events(column="page_url", nodes=NODES_BASIC)
        assert res.df["event"].tolist() == ["page_view:/a/xxx"]

    def test_strip_cgi_removes_query_string(self):
        stream = _make_stream([["u1", "/A/B?ref=home&utm=1", "page_view", "2024-01-01"]])
        res = stream.url_events(column="page_url", nodes=NODES_BASIC)
        assert res.df["event"].tolist() == ["page_view:/a/xxx"]
    ...
    def test_strip_locale_false_keeps_locale_segment(self):
        ...
        res = stream.url_events(column="page_url", nodes=NODES_BASIC, strip_locale=False)
